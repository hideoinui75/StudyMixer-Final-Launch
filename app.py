...
if 'course_folders' not in st.session_state:
    st.session_state.course_folders = ["未分類"] # デフォルトフォルダ
if 'selected_course' not in st.session_state:
    st.session_state.selected_course = "未分類"

# --- FUNCTION: HISTORY CALLBACK (ファイルの先頭に定義) ---
# ★ 修正1-A: 引数に selected_course, report_words, report_vocab, report_keywords を追加 ★
def save_history_entry(generated_text, ai_success, selected_task, difficulty, format_type, professor_focus, summary_length, uploaded_file, selected_course, report_words=None, report_vocab=None, report_keywords=None):
    ...
    try:
        ...
        history_entry = {
            ...
            "task": selected_task,
            "course": selected_course, # ★ 履歴に講義名を追加 ★
        ...
        if st.session_state['analysis_history']:
            ...
            if (last_entry['file_name'] == history_entry['file_name'] and
                ...
                last_entry['course'] == history_entry['course'] and # 講義名もチェック
        ...
        if not is_duplicate:
            ...
            # 履歴の最大件数を制限
            MAX_HISTORY = 100 # フォルダ機能で整理するため件数を緩和
...
with st.sidebar:
    ...
    st.session_state.selected_course = st.selectbox(
        ...
        index=st.session_state.course_folders.index(st.session_state.selected_course),
    ...
    if not filtered_history:
        ...
        history_titles = []
        for i, entry in enumerate(filtered_history): # フィルタリングされたリストを使う
        ...
        selected_history_display = st.selectbox(
            f"過去の分析結果を選択 (フォルダ: {st.session_state.selected_course}):", # 選択中のフォルダ名を表示
        ...
        if selected_history_display != "履歴を選択...":
            try:
                ...
                # --- メインエリアの表示を更新 ---
                # Note: displayed_history_index is the index within the FILTERED list now.
...
def reset_history_selection_on_upload():
    # 履歴選択インデックスをリセットする
    st.session_state.displayed_history_index = None
    # ログ追加後に履歴セレクトボックスの表示が「履歴を選択...」に戻るようにリセット
    st.session_state.history_selectbox_display = "履歴を選択..."


uploaded_file = st.file_uploader(
    f"講義のシラバス、板書、資料（PDF/画像/音声）をアップロード (保存先: **{st.session_state.selected_course}**)",
...
"""
    ...
    # --- 履歴保存処理の呼び出し ---
    # ★ 修正1-B: selected_course を引数に追加して呼び出し ★
    save_history_entry(
        generated_text, 
        ai_success, 
        selected_task, 
        ...
        summary_length, 
        uploaded_file,
        st.session_state.selected_course, # 👈 現在選択中のフォルダ
